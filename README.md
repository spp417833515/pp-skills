...
把「澄清 → 方案 → 执行 → 审查 → 同步 → 归档」固化成 **6 个 slash 技能**(+ `/pp-pipeline` 链路单一信源 · 只读参考),
...
| `/pp-plan-v2` | 方案 | 3 份 MD(对话和澄清/攻击性验收/五类产出)+ 闸门②硬产出(逻辑定位/架构图/底层探测/分层表/影响范围)+ 子代理模拟验证 |
...
| `/pp-pipeline` | 参考 | 链路图 · 命令↔阶段映射 · 目录结构 · 所有权(只读单一信源 · 6 技能引用) |
...
- **一键脚本**:`bash install.sh`(检测 Claude/Codex · 拷 7 技能 · 清旧 v1)
