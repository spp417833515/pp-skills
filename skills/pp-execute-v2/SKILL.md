...
不是 mock 自欺 · 是真实链路(≥1 条真实 URL/KEY)
...
Step 3 · 循环验收(核心 · 直到全绿)
         for 每条 验收标准.md §5 命令式验收:
             ...
             比对「精确期望值」
             ┌ 绿 → 记 ✓ · 下一条
             └ 红 → 定位根因 → 修 → 重跑(禁吞错 / 禁改期望值凑绿)
         §6 真实链路:≥1 条真实 URL/KEY · 跑通 · 非纯 mock

Step 3.5 · 提交前自检(公约自检清单 11 项 · "提交代码前逐项过")
...
| 真实链路无 KEY/URL | 阻断 · 让用户提供(不允许全 mock 蒙混) |
